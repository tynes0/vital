...
public:
#if defined(_WIN64)
	VTL_INLINE static constexpr size_t offset_basis = 14695981039346656037ull;
	VTL_INLINE static constexpr size_t prime = 1099511628211ull;
#else // !defined(_WIN64)
	VTL_INLINE static constexpr size_t offset_basis = 2166136261u;
...
#define VTL_HASH_TRIVIAL_ASSERT(type) VTL_ASSERT(std::is_trivial_v<type>, "only trivial types can be directly hashed");
	...
	template <class _Ty>
	VTL_NODISCARD static size_t append_range(const size_t val, const _Ty* const first, const _Ty* const last) noexcept
	{
		VTL_HASH_TRIVIAL_ASSERT(_Ty);
	...
	template <class _Kty>
	VTL_NODISCARD static size_t append_value(const size_t val, const _Kty& key_value) noexcept
	{
		VTL_HASH_TRIVIAL_ASSERT(_Kty);
		return append_bytes(val, &reinterpret_cast<const unsigned char&>(key_value), sizeof(_Kty));
	}

	template <class _Kty>
	VTL_NODISCARD static size_t hash_representation(const _Kty& key_value) noexcept
	{
		return append_value(offset_basis, key_value);
	}

	template <class _Kty>
	VTL_NODISCARD static size_t hash_array_representation(const _Kty* const first, const size_t count) noexcept
	{
		VTL_HASH_TRIVIAL_ASSERT(_Kty);
...
{
	VTL_NODISCARD size_t operator()(const _Kty key_value) const noexcept (noexcept(hash<_Kty>::do_hash(key_value)))
	{
...
{
	VTL_NODISCARD static size_t do_hash(const _Kty& key_value) noexcept
	{
...
struct hash<float>
{
	VTL_NODISCARD static size_t do_hash(const float key_value) noexcept
	{
		return fnv1a::hash_representation(key_value == 0.0f ? 0.0f : key_value);
	}

	VTL_NODISCARD size_t operator()(const float key_value) const noexcept
	{
...
struct hash<double>
{
	VTL_NODISCARD static size_t do_hash(const double key_value) noexcept
	{
		return fnv1a::hash_representation(key_value == 0.0 ? 0.0 : key_value);
	}

	VTL_NODISCARD size_t operator()(const double key_value) const noexcept
	{
		return do_hash(key_value);
	}
};
...
struct hash<long double>
{
	...
	VTL_NODISCARD size_t operator()(const long double key_value) const noexcept
	{
		return do_hash(key_value);
	}
};
...
{
	VTL_NODISCARD static size_t do_hash(nullptr_t) noexcept
	{
		void* null{};
		return fnv1a::hash_representation(null);
	}

	VTL_NODISCARD size_t operator()(nullptr_t) const noexcept
	{
...
{
	VTL_NODISCARD static size_t do_hash(const std::string& key_value) noexcept
	{
