...
VTL_NODISCARD VTL_CONSTEXPR17 std::_Iter_diff_t<_Iter> distance(_Iter first, _Iter last)
{
	if constexpr (is_vtl_iterator_v<_Iter>)
		return last - first;
	else
	{
...
VTL_CONSTEXPR20 _Fn for_each(_Iter first, _Iter last, _Fn fun)
{
	verify_range(first, last);
	auto ufirst			= get_unwrapped(first);
...
template <class _Iter, class _Ty>
VTL_NODISCARD VTL_CONSTEXPR20 _Iter basic_find(_Iter first, const _Iter last, const _Ty& value)
{
	verify_range(first, last);
	auto ufirst = get_unwrapped(first);
	auto ulast = get_unwrapped(last);
...
VTL_NODISCARD VTL_CONSTEXPR20 _Iter1 find_first_of(_Iter1 first1, const _Iter1 last1, const _Iter2 first2, const _Iter2 last2, _Pr pred) noexcept
{
...
VTL_NODISCARD VTL_CONSTEXPR20 bool all_of(_Iter first, _Iter last, _Pr pred)
{
	verify_range(first, last);
	auto ufirst			= get_unwrapped(first);
	const auto ulast	= get_unwrapped(last);
...
template <class _Iter, class _Pr>
VTL_NODISCARD VTL_CONSTEXPR20 bool any_of(_Iter first, _Iter last, _Pr pred)
{
	verify_range(first, last);
	auto ufirst			= get_unwrapped(first);
	const auto ulast	= get_unwrapped(last);
...
template <class _Iter, class _Pr>
VTL_NODISCARD VTL_CONSTEXPR20 bool none_of(_Iter first, _Iter last, _Pr pred)
{
	verify_range(first, last);
	auto ufirst			= get_unwrapped(first);
	const auto ulast	= get_unwrapped(last);
...
template <class _Iter, class _Pr>
VTL_NODISCARD VTL_CONSTEXPR20 bool all_of_2range(_Iter first1, _Iter first2, size_t n, _Pr pred)
{
	auto ufirst1		= get_unwrapped(first1);
...
template <class _Iter, class _Pr>
VTL_NODISCARD VTL_CONSTEXPR20 bool any_of_2range(_Iter first1, _Iter first2, size_t n, _Pr pred)
{
	auto ufirst1		= get_unwrapped(first1);
	auto ufirst2		= get_unwrapped(first2);
	const auto ulast	= get_unwrapped(first1 + n);
...
template <class _Iter, class _Pr>
VTL_NODISCARD VTL_CONSTEXPR20 bool none_of_2range(_Iter first1, _Iter first2, size_t n, _Pr pred)
{
	auto ufirst1		= get_unwrapped(first1);
	auto ufirst2		= get_unwrapped(first2);
	const auto ulast	= get_unwrapped(first1 + n);
